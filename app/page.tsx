...
export default function HomePage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setError(null);
    const base = platform === "github" ? `/${trimmed}` : `/lc/${trimmed}`;
    router.push(country ? `${base}?country=${country}` : base);
  }
    ...
    <main className="mow-lines relative min-h-screen overflow-hidden">
      ...
      <section className="relative z-10 mx-auto flex max-w-3xl flex-col items-center px-6 pb-12 pt-6 text-center sm:pt-12">
        ...
        >
          ...
          <div className="flex w-full items-center gap-2 rounded-md border border-chalk/15 bg-pitch/50 px-3 py-2">
            <span className="text-sm">{country ? countryFlag(country) : "🌐"}</span>
            <select
              value={country}
              onChange={(e) => setCountry(e.target.value)}
              className="w-full bg-transparent py-1 font-mono text-xs text-chalk/70 outline-none [&>option]:bg-pitch [&>option]:text-chalk"
            >
              <option value="">Add your country (optional)</option>
              {COUNTRIES.map((c) => (
