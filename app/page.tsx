    ...
    <main className="mow-lines relative min-h-screen overflow-hidden">
      ...
      <section className="relative z-10 mx-auto flex max-w-3xl flex-col items-center px-6 pb-12 pt-6 text-center sm:pt-12">
        ...
        <motion.a
          ...
          onClick={(e) => {
            e.preventDefault();
            document.getElementById("career-card")?.scrollIntoView({ behavior: "smooth" });
          }}
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ delay: 0.4, duration: 0.4 }}
          className="mt-6 flex flex-col items-center gap-1 font-body text-xs text-chalk/40 transition hover:text-bail"
        >
          <span>See your Career Card too</span>
          <motion.span animate={{ y: [0, 4, 0] }} transition={{ duration: 1.4, repeat: Infinity, ease: "easeInOut" }} aria-hidden>
            ↓
          </motion.span>
        </motion.a>
      </section>

      {/* Two ways to see your profile — Career Card gets real visual weight here (accent
          border, badge, its preview embedded directly in the box) instead of reading as
          an equal-weight twin to the Cricket Card box next to it. */}
      <section id="career-card" className="relative z-10 mx-auto max-w-5xl px-6 pb-16 pt-8">
        <motion.p
          initial={{ opacity: 0, y: 8 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="text-center font-display text-xs uppercase tracking-widest text-leather"
        >
          Two ways to see your developer profile
        </motion.p>
        ...
        <div className="mt-8 grid gap-6 md:grid-cols-5">
          <motion.div
            ...
            className="rounded-xl border border-chalk/10 bg-pitch/60 p-6 md:col-span-2"
          >
            <p className="font-display text-lg font-black uppercase italic text-chalk">Cricket Card</p>
            <p className="mt-1 font-body text-xs uppercase tracking-widest text-bail">How you play</p>
            <p className="mt-3 font-body text-sm leading-relaxed text-chalk/60">
              Your developer stats, turned into a cricket player card. Fun, instant, and the fastest way to share
              what you&apos;ve built.
            </p>
            <p className="mt-4 font-body text-xs text-chalk/40">Already the flow above — just enter a GitHub username.</p>
          </motion.div>
          ...
          <motion.div
            ...
            className="relative overflow-hidden rounded-xl border border-bail/40 bg-gradient-to-b from-bail/[0.06] to-pitch/60 p-6 shadow-[0_0_40px_-12px_rgba(217,169,59,0.35)] md:col-span-3"
          >
            <span className="absolute right-4 top-4 rounded-full bg-bail px-2.5 py-0.5 font-display text-[10px] font-bold uppercase tracking-widest text-pitch">
              New
            </span>
            <p className="font-display text-lg font-black uppercase italic text-chalk">Career Card</p>
            <p className="mt-1 font-body text-xs uppercase tracking-widest text-bail">What you&apos;ve built</p>
            <p className="mt-3 max-w-sm font-body text-sm leading-relaxed text-chalk/60">
              Combine your GitHub, CV, and coding profile into one evidence-backed profile — strengths, gaps, and
              claimed vs. public evidence, side by side.
            </p>
            <a
            ...
            <div className="mt-6 border-t border-bail/15 pt-5">
              <CareerCardPreview />
            </div>
          </motion.div>
        </div>
